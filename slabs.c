...
typedef struct {
    unsigned int size;      /* sizes of items */ //slab分配器分配的item的大小
    unsigned int perslab;   /* how many items per slab */ //每个slab分配器能分配中多少个item

    void *slots;           /* list of item ptrs */ //指向空闲item链表
    unsigned int sl_curr;   /* total free items in list */ //空闲item个数

    //这个是已经分配了内存的slabs个数 list_size是这个slabs数组(slab_list)的大小
    unsigned int slabs;     /* how many slabs were allocated for this class */ //这个类型的slabclass_t可用的slab分配器个数

    //slab数组 数组的每一个元素就是一个slab分配器 这些分配器都分配相同尺寸的内存
    void **slab_list;       /* array of slab pointers */
    unsigned int list_size; /* size of prev array */ //slabs数组的大小

    size_t requested; /* The number of requested bytes */ //本slabclass_t分配出去的字节数
} slabclass_t;
...
unsigned int slabs_clsid(const size_t size) {
    int res = POWER_SMALLEST;//初始化为1

    if (size == 0 || size > settings.item_size_max)//如果size为0 或 大于配置
        return 0;
    while (size > slabclass[res].size)//因为slabclass数组中各个元素分配的item大小是升序的 所以从小到大直接判断即可在数组中找到最小且满足要求的元素
...
void slabs_init(const size_t limit, const double factor, const bool prealloc, const uint32_t *slab_sizes) {
    ...
    mem_limit = limit;//用户设置或默认的内存最大限制
    //用户要求预分配一块大的内存 以后需要内存 就向这块内存申请
    if (prealloc) {//默认prealloc是false
        /* Allocate everything in a big chunk with malloc */
        mem_base = malloc(mem_limit);//指向预先分配的内存
        if (mem_base != NULL) {
            mem_current = mem_base;//指向还可以使用的内存的开始位置
            mem_avail = mem_limit;//指明还有多少内存是可以使用的
        } else {
    ...
    while (++i < MAX_NUMBER_OF_SLAB_CLASSES-1) {
        if (slab_sizes != NULL) {//判断slab_sizes是为NULL
            if (slab_sizes[i-1] == 0)//判断是为0
                break;
            size = slab_sizes[i-1];//size重新赋值
        } else if (size >= settings.slab_chunk_size_max / factor) {
            break;
        }
        /* Make sure items are always n-byte aligned */ //确保item总是n-byte对齐
        if (size % CHUNK_ALIGN_BYTES)//8字节对齐
            size += CHUNK_ALIGN_BYTES - (size % CHUNK_ALIGN_BYTES);

        slabclass[i].size = size;//slabclass[i]的slab分配器能分配item的大小
        slabclass[i].perslab = settings.slab_page_size / slabclass[i].size;//slabclassp[i]的slab分配器最多能分配多少个item
        if (slab_sizes == NULL)//slab_size是为NULL
            size *= factor;//size增加 乘以因子factor
    ...
    power_largest = i;//power_largest记录slabclass数组已分配的下标i
    slabclass[power_largest].size = settings.slab_chunk_size_max;//设置为配置的最大size
    slabclass[power_largest].perslab = settings.slab_page_size / settings.slab_chunk_size_max;//最多能分配多少个item
    if (settings.verbose > 1) {
        fprintf(stderr, "slab class %3d: chunk size %9u perslab %7u\n",
                i, slabclass[i].size, slabclass[i].perslab);
    }
...
static void slabs_preallocate (const unsigned int maxslabs) {//预分配内存
    int i;
    ...
    for (i = POWER_SMALLEST; i < MAX_NUMBER_OF_SLAB_CLASSES; i++) {//遍历slabclass数组
...
static int grow_slab_list (const unsigned int id) {//增加slab_list成员指向的内存 也就是增大slab_list数组 使得可以有更多的slab分配器 除非内存分配失败 否则都是返回1 无论是否真正增大了
    slabclass_t *p = &slabclass[id];
    if (p->slabs == p->list_size) {//如果分配器分配的数量等于数组大小 说明用完了之前申请到的slab_list数组的所有元素 则将扩大
...
static void split_slab_page_into_freelist(char *ptr, const unsigned int id) {
    slabclass_t *p = &slabclass[id];//下标为id
    int x;
    for (x = 0; x < p->perslab; x++) {//slabclass[id]中每个slab有perslab个item 即将ptr指向的内存分为p->perslab个item 其中每个item大小为p->size
...
static void *get_page_from_global_pool(void) {
    slabclass_t *p = &slabclass[SLAB_GLOBAL_PAGE_POOL];//slabclass[0]
    if (p->slabs < 1) {
        return NULL;
    }
    char *ret = p->slab_list[p->slabs - 1];//获取slabclass[0]数组中的一个list
    p->slabs--;//数量减一
    return ret;
}

static int do_slabs_newslab(const unsigned int id) {//slabclass_t中的slab数目慢慢增多 该函数的作用就是为slabclass_t申请多一个slab 参数id指明是slabclass数组中的那个slabclass_t
    slabclass_t *p = &slabclass[id];
    slabclass_t *g = &slabclass[SLAB_GLOBAL_PAGE_POOL];
    int len = (settings.slab_reassign || settings.slab_chunk_size_max != settings.slab_page_size)//settings.slab_reassign的默认值为false 这里就是采用false
    ...
    if ((grow_slab_list(id) == 0) ||
        (((ptr = get_page_from_global_pool()) == NULL) &&//ptr或是get_page_from_global_pool返回
        ((ptr = memory_allocate((size_t)len)) == 0))) {//ptr或是memory_allocate返回

        MEMCACHED_SLABS_SLABCLASS_ALLOCATE_FAILED(id);
        return 0;
    }

    memset(ptr, 0, (size_t)len);//初始化ptr指向的那块内存
    split_slab_page_into_freelist(ptr, id);//将ptr指向的那块内存分成一个一个itme

    p->slab_list[p->slabs++] = ptr;//ptr归属 p即slabclass[id]数组下标为p->slabs指向的list
...
static void do_slabs_free(void *ptr, const size_t size, unsigned int id) {
    slabclass_t *p;
    item *it;//具体item结构是 typedef struct _stritem在memcached.h中定义
...
static void *memory_allocate(size_t size) {
    ...
    if (mem_base == NULL) {
        ...
        ret = malloc(size);//没有使用预分配内存 则直接使用malloc申请size大小的内存
    } else {
        ret = mem_current;//使用预分配内存当前可用内存起始位置

        if (size > mem_avail) {//可用大小
            return NULL;
        }

        /* mem_current pointer _must_ be aligned!!! */
        if (size % CHUNK_ALIGN_BYTES) {//8字节对齐
            size += CHUNK_ALIGN_BYTES - (size % CHUNK_ALIGN_BYTES);//更新size变量
        }

        mem_current = ((char*)mem_current) + size;//更新预分配内存当前可用起始位置
        if (size < mem_avail) {//size与预分配内存可用大小比较 更新预分配大小
            mem_avail -= size;
        } else {
            mem_avail = 0;
        }
    }
    mem_malloced += size;//更新
